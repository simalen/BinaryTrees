...
void testTree(BSTree tree)
{
    ...
    //Tradet ska vara tomt frŒn borjan
    assert(isEmpty(tree));
    //Tester pŒ tomt trŠd
    assert(!find(tree, 7)); //leta i tomt trŠd
    removeElement(&tree, 7); //ta bort frŒn tomt trŠd - programmet ska inte krasha
    assert(depth(tree) == 0 && minDepth(tree) == 0);
    balanceTree(&tree); //balansera ett tomt trŠd - programmet ska inte krasha
    ...
    /*Avkommentera rad 120-130 om du vid borttagning av nod med tvŒ barn vŠljer det stšrsta i vŠnster deltrŠd som ersŠttare (8 ersŠtter dŒ 9 i det hŠr testet), jŠmfšr dina utskrifter av trŠdet med de utskrifter som visar hur trŠdet bšr se ut. Om du behšver kan du justera mellanslagen i utskrifterna sŒ att formateringen ser bra ut i din konsoll. */
    /*
    printf("\nPreorder\nYour tree:    ");
    printPreorder(tree, stdout);
